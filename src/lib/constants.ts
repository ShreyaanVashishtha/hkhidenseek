...
export const MTR_MAP_PLACEHOLDER_URL = "https://placehold.co/800x600.png"; // Admin should be able to change this
...
export const GAME_RULES = {
  ...
  gamePhases: {
    ...
    phases: [
      ...
      { name: "2. Seeking Phase (up to 2 hours or until capture)", description: "Seekers begin moving via the MTR. Seekers complete challenges to earn coins. Coins are used to purchase questions that narrow down the hider's location." }
    ]
  },
  hidingRules: {
    title: "Hiding Rules",
    rules: [
      ...
      "Hiders may purchase curse dice to interfere with Seekers."
    ]
  },
  challenges: {
    title: "Challenges (Seekers Only)",
    rules: [
      "Seekers complete physical, or location-based tasks to earn coins.",
  ...
  coins: {
    title: "Coins",
    rules: ["Seekers earn coins by completing challenges. Coins are spent to ask questions."]
  },
  questionRules: {
  ...
  curseDiceRules: {
    ...
    rules: [
      ...
      "Curse dice can only be used two times per round, per hider team.",
...
export const QUESTION_OPTIONS: QuestionOption[] = [
  { id: 'q_radar', name: 'Radar', category: 'Radar', cost: 30, description: "Determine if hiders are near a location. Must be answered truthfully with 'Yes' or 'No'.", seekerPrompt: "e.g., Tsim Sha Tsui Station, Tsuen Wan Line, Hong Kong Island, New Territories, station connecting to East Rail Line, station with MTR interchange, station above ground.", icon: MapPin },
  { id: 'q_precision', name: 'Precision', category: 'Precision', cost: 10, description: "Yes/no questions to narrow down exact characteristics.", seekerPrompt: "e.g., Does your zone include a park? Near coastal area? District starts with 'S'? Includes a university? Connected to 2+ MTR lines?", icon: Target },
  { id: 'q_relative', name: 'Relative', category: 'Relative', cost: 40, description: "Compare a trait of seekers' current position with hiders'.", seekerPrompt: "e.g., Higher elevation? Line with fewer stations? MTR station closer to Central? Nearest MTR station north of us? More residential area?", icon: GitCompareArrows },
  { id: 'q_photo', name: 'Photo', category: 'Photo', cost: 15, description: "Hiders send a real-time photo of something nearby (no direct clues). Disabled if seekers in zone.", seekerPrompt: "e.g., Nearest shop? Tree/park? Something blue? Building >10 floors? Any advertisement? Tallest building seen?", icon: Camera, disabledCondition: (gameState, team) => { /* complex logic: if seekers in hider zone */ return false; } },
  { id: 'q_scan', name: 'Scan', category: 'Scan', cost: 20, description: "Ask 'Are you in [specific region]?' (1 per 30 mins). Truthful response within 2 mins.", seekerPrompt: "e.g., Kowloon, Island Line, Southern District.", icon: ScanLine }
];
